"""Database layer – Motor async MongoDB wrapper.All collections live here; cogs call only these methods."""

from future import annotations

import randomimport stringfrom datetime import datetime, timezonefrom typing import Any

from motor.motor_asyncio import AsyncIOMotorDatabase

def _now() -> datetime:return datetime.now(timezone.utc)

def _gen_case_id() -> str:"""Generate a unique 5-digit numeric case ID string."""return str(random.randint(10000, 99999))

class Database:def init(self, db: AsyncIOMotorDatabase) -> None:self.db = db# Collectionsself.warnings   = db["warnings"]self.cases      = db["cases"]self.evidence   = db["evidence"]self.reports    = db["reports"]self.appeals    = db["appeals"]self.queue_bans = db["queue_bans"]self.punishments = db["punishments"]   # general timeouts / queue locks
