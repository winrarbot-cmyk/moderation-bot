"""
Cog: Evidence
Commands: !evidence add [case_id], !evidence delete [case_id]
"""
...
from utils.helpers import (
    COLOUR_INFO, COLOUR_SUCCESS, COLOUR_ERROR, COLOUR_WARNING,
...
class EvidenceModal(discord.ui.Modal, title="Add Evidence"):
    link = discord.ui.TextInput(
        label="Evidence Link",
        placeholder="YouTube / Medal / Twitch / Streamable / any URL",
        required=False,
        max_length=500,
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        self.ev_link   = self.link.value.strip()
        self.ev_notes  = self.notes.value.strip()
        self.submitted = True
        await interaction.response.send_message(
            "📎 Modal received. You have **5 minutes** to upload additional files (images, videos, attachments) "
            "by sending them as messages in this channel. Type `done` or wait for the timer to finish.",
            ephemeral=False,
        )


class EvidenceDeleteView(discord.ui.View):
    """Shows a select menu to choose which evidence entry to delete."""

    def __init__(self, evidence_items: list[dict], author_id: int):
    ...
    @discord.ui.select(placeholder="Select evidence to delete…")
    async def select_menu(self, interaction: discord.Interaction, select: discord.ui.Select):
        self.to_delete = select.values[0]
        self.stop()
...
class Evidence(commands.Cog):
    ...
    @commands.group(name="evidence", invoke_without_command=False)
    @staff_only()
    ...
    @staff_only()
    async def evidence_add(self, ctx: commands.Context, case_id: str):
        """
        Open a modal to add evidence to a case, then collect file uploads for 5 minutes.
        Usage: !evidence add 51832
        """
        # Verify case exists
    ...
    @staff_only()
    async def evidence_delete(self, ctx: commands.Context, case_id: str):
        """
        Remove an evidence entry from a case via a selection menu.
        Usage: !evidence delete 51832
        """
        items = await self.db.get_evidence(case_id)
        if not items:
            await ctx.send(f"❌ No evidence found for case **#{case_id}**.", delete_after=8)
            return
