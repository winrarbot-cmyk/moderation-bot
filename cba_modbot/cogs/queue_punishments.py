"""Cog: Queue PunishmentsCommands: !queueban, !unqueueban, !gentimeout"""
...
from config.settings import CONFIGfrom utils.helpers import (COLOUR_ERROR, COLOUR_SUCCESS, COLOUR_INFO, COLOUR_WARNING,staff_only, base_embed, format_duration, parse_duration,)

log = logging.getLogger("cba_bot.queue_punishments")

class QueuePunishments(commands.Cog):def init(self, bot: commands.Bot):self.bot = bot

@propertydef db(self):return self.bot.db

── !queueban @user [duration] [reason] ──────────────────────────────────

@commands.command(name="queueban")@staff_only()async def queue_ban(self,ctx: commands.Context,member: discord.Member,duration: str,*,reason: str = "No reason provided.",):"""Restrict a user from all queue channels.Does NOT create roles – uses channel permission overrides.Usage: !queueban @user 7d Smurfing!queueban @user permanent Match Manipulation"""guild_id = ctx.guild.id
...
@commands.command(name="unqueueban")@staff_only()async def unqueue_ban(self, ctx: commands.Context, member: discord.Member):"""Remove an active queue ban and restore access to queue channels.Usage: !unqueueban @user"""guild_id = ctx.guild.id
...
@commands.command(name="gentimeout")@staff_only()async def general_timeout(self,ctx: commands.Context,member: discord.Member,duration: str,*,reason: str = "No reason provided.",):"""Apply Discord's native timeout to a user. No role creation.Usage: !gentimeout @user 24h Toxicity"""try:duration_minutes = parse_duration(duration)except ValueError:await ctx.send("❌ Invalid duration. Examples: 30m, 2h, 24h, 7d.",delete_after=8,)return
...
async def untimeout(self, ctx: commands.Context, member: discord.Member, *, reason: str = "No reason provided."):
    """Immediately remove a Discord timeout from a user and update the DB record.
    Usage: !untimeout @user
           !untimeout @user Issued in error"""
    # Check if the member is actually timed out
    if not member.is_timed_out():
        await ctx.send(
            f"⚠️ {member.mention} is not currently timed out.",
            delete_after=8,
        )
        return
    ...
    try:
        ...
        await ctx.send("❌ I don't have permission to remove this timeout.", delete_after=8)
        return
    except discord.HTTPException as e:
        await ctx.send(f"❌ Failed to remove timeout: {e}", delete_after=8)
        return

    # Mark the most recent active general_timeout punishment as inactive
    await self.db.punishments.update_one(
        ...
        {"$set": {
            ...
            "removed_by_name":  str(ctx.author),
            "removal_reason":   reason,
        }},
        # sort not supported in update_one; we update one – the latest is fine here
    )

    embed = base_embed(
        "✅ Timeout Removed",
        f"{member.mention}'s Discord timeout has been lifted immediately.",
        colour=COLOUR_SUCCESS,
    )
    embed.add_field(name="Reason", value=reason, inline=False)
    embed.set_footer(text=f"Removed by {ctx.author.display_name}")
    embed.timestamp = datetime.now(timezone.utc)
    await ctx.send(embed=embed)
    log.info("Timeout removed for %s by %s – reason: %s", member, ctx.author, reason)
...
async def unban(self, ctx: commands.Context, user: discord.User, *, reason: str = "No reason provided."):
    """Unban a user from the server (optional – use when a server ban was issued).
    Usage: !unban @user
    ...
    try:
        ...
        await ctx.send(
            f"⚠️ {user.mention} ({user}) is not currently banned from this server.",
            delete_after=8,
        )
        return
    except discord.Forbidden:
        await ctx.send("❌ I don't have permission to unban members.", delete_after=8)
        return
    except discord.HTTPException as e:
        await ctx.send(f"❌ Failed to unban: {e}", delete_after=8)
        return
    ...
    embed.timestamp = datetime.now(timezone.utc)
    await ctx.send(embed=embed)
    log.info("User %s (%s) unbanned by %s – reason: %s", user, user.id, ctx.author, reason)
