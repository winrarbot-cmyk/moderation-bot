...
from utils.helpers import (
    COLOUR_WARNING, COLOUR_ERROR, COLOUR_SUCCESS, COLOUR_INFO,
    staff_only, suggest_reason, categorise_reason, is_queue_related,
    format_duration, base_embed, warning_summary_embed,
)
...
class ConfirmReasonView(discord.ui.View):
    """Shown when the entered reason doesn't match a known infraction."""

    def __init__(self, original: str, suggested: str, author_id: int):
        super().__init__(timeout=60)
        self.original  = original
        self.suggested = suggested
        self.author_id = author_id
        self.choice: str | None = None   # set on interaction
    ...
    @discord.ui.button(label="✅ Confirm Original Reason", style=discord.ButtonStyle.success)
    async def keep_original(self, interaction: discord.Interaction, _: discord.ui.Button):
        self.choice = self.original
        self.stop()
        await interaction.response.defer()

    @discord.ui.button(label="❌ Use Suggested Reason", style=discord.ButtonStyle.danger)
    async def use_suggested(self, interaction: discord.Interaction, _: discord.ui.Button):
        self.choice = self.suggested
        self.stop()
        await interaction.response.defer()


class ApplyPunishmentView(discord.ui.View):
    """Shown when a punishment recommendation exists."""
    ...
    async def do_apply(self, interaction: discord.Interaction, _: discord.ui.Button):
        self.apply = True
        self.stop()
        await interaction.response.defer()

    @discord.ui.button(label="❌ Warning Only", style=discord.ButtonStyle.secondary)
    async def warning_only(self, interaction: discord.Interaction, _: discord.ui.Button):
        self.apply = False
        self.stop()
        await interaction.response.defer()


class EvidenceButtonView(discord.ui.View):
    """Persistent view with a 'View Evidence' button on warns embeds."""
    ...
    @discord.ui.button(
        ...
        custom_id="view_evidence",
    )
    async def view_evidence(self, interaction: discord.Interaction, _: discord.ui.Button):
        db = interaction.client.db
        items = await db.get_evidence(self.case_id)
        if not items:
            await interaction.response.send_message("No evidence attached to this case.", ephemeral=True)
            return
...
class Warnings(commands.Cog):
    ...
    async def warn(self, ctx: commands.Context, member: discord.Member, *, reason: str):
        """
        Warn a user. Automatically escalates punishment if the reason matches a known infraction.
    ...
    async def _remove_queue_access(self, guild: discord.Guild, member: discord.Member) -> None:
        queue_channel_names = CONFIG["channels"]["queue_channels"]
        for ch_name in queue_channel_names:
            channel = discord.utils.get(guild.channels, name=ch_name)
            if channel:
                try:
                    await channel.set_permissions(member, view_channel=False)
                except discord.Forbidden:
                    log.warning("Cannot set permissions in %s", channel.name)

    # ── Helper: community warning escalation ──────────────────────────────────
    async def _check_community_escalation(
        self, ctx: commands.Context, member: discord.Member, guild_id: int
    ) -> None:
        count = await self.db.count_active_community_warnings(member.id, guild_id)
        escalation = CONFIG["community_escalation"]
        action = None
        for rule in reversed(escalation):
    ...
    async def _check_escalation_alerts(
        self,
        ctx: commands.Context,
        member: discord.Member,
        guild_id: int,
        ...
        alert_channel = discord.utils.get(ctx.guild.text_channels, name=alert_channel_name)
        ...
        for rule in CONFIG["alert_rules"]:
            ...
            if count >= rule["count"]:
                ...
                ping = mod_role.mention if mod_role else "@Moderator"
                embed = base_embed(
                    "🚨 Escalation Alert",
                    f"{ping}\n"
    ...
    async def warns(self, ctx: commands.Context, member: discord.Member):
        """
        Display full warning history for a user.
        Usage: !warns @user
        """
        guild_id = ctx.guild.id
        warnings    = await self.db.get_warnings(member.id, guild_id)
        punishments = await self.db.get_all_punishments(member.id, guild_id)
        queue_bans  = await self.db.queue_bans.find(
            {"user_id": member.id, "guild_id": guild_id}
        ).sort("created_at", -1).to_list(length=None)
        appeals = await self.db.appeals.find(
            {"user_id": member.id, "guild_id": guild_id}
        ).sort("created_at", -1).to_list(length=None)
